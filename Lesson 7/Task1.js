...
const config = {
    ...
    validate() {
        ...
        if (this.settings.rowsCount < 10 || this.settings.rowsCount > 30) {
            ...
            result.errors.push('Wrong settings, the value rowsCount should be in range [10, 30].');
        }

        if (this.settings.colsCount < 10 || this.settings.colsCount > 30) {
            result.isValid = false;
            result.errors.push('Wrong settings, the value colsCount should be in range [10, 30].');
        }

        if (this.settings.speed < 1 || this.settings.speed > 10) {
            result.isValid = false;
            result.errors.push('Wrong settings, the value speed should be in range [1, 10].');
        }

        if (this.settings.winFoodCount < 5 || this.settings.winFoodCount > 50) {
            result.isValid = false;
            result.errors.push('Wrong settings, the value winLength should be in range [5, 50].');
        }
...
const game = {
    ...
    play() {
        ...
        // Меняем название кнопки в меню на "Stop" и делаем ее активной.
    ...
    stop() {
        ...
        // Меняем название кнопки в меню на "Start" и делаем ее активной.
